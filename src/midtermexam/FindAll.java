...
public class FindAll {
	/* Phần mở rộng là .docx, .txt, .xlsx,... */
	public void findAll(String path, String ext1, String ext2) throws IOException {
		List<String> list = new ArrayList<String>();// list de luu nhung file tim dc
		...
		if (!file.exists())
			...
			for (File f : listFile) {
				if (f.isDirectory())//neu la thu muc thi goi đệ quy tìm trong thư mục đó
					findAll(f.getCanonicalPath(), ext1, ext2);
	...
	public static void main(String[] args) throws IOException {
		...
		FindAll e3 = new FindAll();
		e3.findAll(path, ".docx", ".xlsx");
	}
